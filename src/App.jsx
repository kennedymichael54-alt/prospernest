...
function BudgetTab({ data }) {
  return (
    <div style={{ animation: 'slideIn 0.3s ease' }}>
      <h2 style={{ fontSize: '24px', fontWeight: '600', marginBottom: '24px' }}>🎯 Budget Overview</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
        {data.budget.categories.map((cat, i) => {
          ...
          return (
            <div key={i} style={{ background: 'rgba(30, 27, 56, 0.8)', backdropFilter: 'blur(20px)', borderRadius: '16px', padding: '20px', border: '1px solid rgba(255,255,255,0.1)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <span style={{ fontSize: '24px' }}>{cat.emoji}</span>
                  <span style={{ fontWeight: '600' }}>{cat.name}</span>
                </div>
                <span style={{ color: isOver ? '#EF4444' : 'rgba(255,255,255,0.6)', fontSize: '14px' }}>
                  ${cat.spent} / ${cat.budget}
                </span>
              </div>
              <div style={{ height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{ width: `${Math.min(pct, 100)}%`, height: '100%', background: isOver ? '#EF4444' : cat.color, borderRadius: '4px' }} />
              </div>
              {isOver && (
                <div style={{ fontSize: '12px', color: '#EF4444', marginTop: '8px' }}>
                  😤 ${(cat.spent - cat.budget).toFixed(0)} over budget!
                </div>
              )}
            </div>
          );
...
function BillsTab({ data, setData }) {
  const toggleBillPaid = (id) => {
    setData(prev => ({
      ...
      bills: prev.bills.map(b => 
        b.id === id ? { ...b, status: b.status === 'paid' ? 'upcoming' : 'paid' } : b
      )
    }));
  };
  ...
  return (
    <div style={{ animation: 'slideIn 0.3s ease' }}>
      <h2 style={{ fontSize: '24px', fontWeight: '600', marginBottom: '24px' }}>📅 Bills & Payments</h2>
      ...
      <div style={{ display: 'grid', gap: '12px' }}>
        {data.bills.map(bill => {
          ...
          return (
            <div key={bill.id} style={{ background: 'rgba(30, 27, 56, 0.8)', backdropFilter: 'blur(20px)', borderRadius: '16px', padding: '20px', border: isOverdue ? '1px solid rgba(239, 68, 68, 0.3)' : '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <span style={{ fontSize: '28px' }}>{bill.emoji}</span>
                <div>
                  <div style={{ fontWeight: '600', fontSize: '16px' }}>{bill.name}</div>
                  <div style={{ fontSize: '13px', color: isOverdue ? '#EF4444' : isPaid ? '#10B981' : 'rgba(255,255,255,0.5)' }}>
                    {isPaid ? '✅ Paid' : isOverdue ? '😤 Overdue!' : `Due ${bill.dueDate.toLocaleDateString()}`}
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <span style={{ fontSize: '20px', fontWeight: '700' }}>${bill.amount}</span>
                <button onClick={() => toggleBillPaid(bill.id)} style={{ padding: '8px 16px', background: isPaid ? 'rgba(16, 185, 129, 0.2)' : 'linear-gradient(135deg, #8B5CF6, #EC4899)', border: 'none', borderRadius: '8px', color: 'white', cursor: 'pointer', fontSize: '14px' }}>
...
function GoalsTab({ data, setData }) {
  ...
  return (
    <div style={{ animation: 'slideIn 0.3s ease' }}>
      <h2 style={{ fontSize: '24px', fontWeight: '600', marginBottom: '24px' }}>🎯 Financial Goals</h2>
      
      {/* Add New Goal */}
      <div style={{ background: 'rgba(30, 27, 56, 0.8)', backdropFilter: 'blur(20px)', borderRadius: '16px', padding: '20px', border: '1px solid rgba(139, 92, 246, 0.3)', marginBottom: '24px' }}>
        <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '16px' }}>✨ Add New Goal</h3>
        <div style={{ display: 'flex', gap: '12px' }}>
          <select value={newGoal.emoji} onChange={e => setNewGoal({ ...newGoal, emoji: e.target.value })} style={{ padding: '12px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', color: 'white', fontSize: '20px' }}>
            {['🎯', '🏠', '🚗', '✈️', '💍', '📱', '🎓', '💰', '🛡️'].map(e => <option key={e} value={e}>{e}</option>)}
          </select>
          <input placeholder="Goal name" value={newGoal.name} onChange={e => setNewGoal({ ...newGoal, name: e.target.value })} style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', color: 'white' }} />
          <input placeholder="Target amount" type="number" value={newGoal.target} onChange={e => setNewGoal({ ...newGoal, target: e.target.value })} style={{ width: '150px', padding: '12px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', color: 'white' }} />
